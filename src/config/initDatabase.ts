...
const initDatabase = (): void => {
  ...
  `,
    (err: Error | null) => {
      ...
      // Create comments table after products table
      db.run(
        `
    ...
    `,
        (err: Error | null) => {
          ...
          // Insert sample data after both tables are created
          setTimeout(() => {
            insertSampleData();
          }, 100);
        }
      ...
      function (this: any, err: Error | null) {
        if (err) {
          ...
          if (productsInserted === sampleProducts.length) {
            // Insert sample comments after all products are inserted
            setTimeout(() => {
...
const insertSampleComments = (): void => {
